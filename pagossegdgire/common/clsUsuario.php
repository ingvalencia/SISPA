...
class clsUsuario extends clsCatalogos{
	...
	public function updUsuario($id_usuario, $lstParam=array()){
		...
		foreach($lstParam as $col => $val){
			switch($col){
				...
				case "passwd": $sets .= ", $col = md5('$val') "; break;
	...
	public function isUserValid($login, $pwd){
		...
		$query = "".
		"SELECT id_usuario, login, passwd, vigente FROM usuarios WHERE vigente = 1 AND login = '$login' ";		
		...
		if(!$resp->num_rows){
			$resp->close();
			return (object)array("count" => 0, "valid" => 0);
		}
		...
		if($usr->passwd != md5($pwd)){
			return (object)array("count" => 1, "valid" => 0);
		}
		
		return (object)array("count" => 1, "valid" => 1, 'tipoUsuario' => 'D');
	...
	public function search_usuarios($lstParam = array()){
		...
		foreach($lstParam as $col => $val){
			...
			switch($col){
				...
				case "nombre": $where.= " AND CONCAT(nombre_usr, ' ', ap_pat_usr, ' ', ap_mat_usr) like '%$val%' "; break;
			}
		}
