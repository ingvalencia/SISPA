...
class clsParametros extends clsConexion{
	...
	*/
	...
	public function searchPlantelSybase(){
		...
		$sybase_link = mssql_connect($CONFIG->sybase->host, $CONFIG->sybase->user, $CONFIG->sybase->pass);
		
		if(!$sybase_link){
			return false;
		}
	
		if(!mssql_select_db($CONFIG->sybase->db, $sybase_link)){
			return false;	
		}
		...
		$result = mssql_query($query, $sybase_link);
