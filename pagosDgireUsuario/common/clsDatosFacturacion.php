...
class clsDatosFacturacion extends clsCatalogos{
	...
	public function search_datos_facturacion($lstParam = array()){
		...
		INNER JOIN Mercurioz_dgirecat.ct_estados e ON f.id_edo = e.id_edo
		INNER JOIN Mercurioz_dgirecat.ct_municipios m ON f.id_municipio = m.id_municipio
		INNER JOIN Mercurioz_dgirecat.ct_ciudades c ON f.id_ciudad = c.id_ciudad
		INNER JOIN solicitantes_pago s ON f.id_solicitante = s.id_solicitante
		LEFT OUTER JOIN Mercurioz_dgirecat.ct_colonias col ON col.id_colonia = f.id_colonia 
		";
