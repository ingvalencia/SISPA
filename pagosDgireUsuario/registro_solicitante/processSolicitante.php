...
if(!$solicitantes){
	...
	$json["error"] = "si";
	$json["msg"] = "Error B001: No se pudo conectarse con el servidor, si el problema persiste comuniquese con el administrador";
...
if($opt == "getIni"){
	...
	$parametros = new clsParametros($solicitantes->getLinkMysql());
	$terminos = $parametros->get_terminos_condiciones();
	$resp = $solicitantes->getPerfil();

	if(!$resp){
		$json["error"] = true;
		$json["msg"] = "Error B002: No se pudo conectarse con el servidor, si el problema persiste comuniquese con el administrador";
		$json["debug"] = $solicitantes->getError();	
		die(json_encode($json));
	}
	
	$json["error"] = false;
	$json["perfiles"] = $resp->perfiles;
	$json["terminos"] = $terminos->terminos;
	die(json_encode($json));
}
...
if($opt == "validEmail"){
	...
	if(!$resp){
		$json["error"] = true;
		$json["msg"] = "Error B003: No se pudo conectarse con el servidor, si el problema persiste comuniquese con el administrador";
		$json["debug"] = $solicitantes->getError();	
		die(json_encode($json));
	}
...
if($opt == "validRFC"){
	...
	if(!$resp){
		$json["error"] = true;
		$json["msg"] = "Error B004: No se pudo conectarse con el servidor, si el problema persiste comuniquese con el administrador";
		$json["debug"] = $solicitantes->getError();	
		die(json_encode($json));
	}
...
if($opt == "addSolicitante"){
	...
	/*enviar cadena por correo*/
		...
		$contenido=$enviacorreo->imprimeFormulariobodyHTML($nomTotal,$url_val);
...
if($opt == "getColonia"){
	...
	if(!$resp){
		$json["error"] = true;
		$json["msg"] = "Error B0010: No se pudo conectarse con el servidor, si el problema persiste comuniquese con el administrador";
		$json["debug"] = $solicitantes->getError();	
		die(json_encode($json));
	}
